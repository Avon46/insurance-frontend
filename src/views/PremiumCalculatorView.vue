...
const multiplier = computed(() =>
  result.value ? result.value.ageCoefficient * result.value.riskCoefficient : 0,
)
const multiplierLabel = computed(() => `${multiplier.value.toFixed(2)}×`)

const ageCoefPercent = computed(() =>
  result.value ? Math.min(100, Math.round((result.value.ageCoefficient / MAX_COEFFICIENT) * 100)) : 0,
)
const riskCoefPercent = computed(() =>
  result.value ? Math.min(100, Math.round((result.value.riskCoefficient / MAX_COEFFICIENT) * 100)) : 0,
)
...
const insightText = computed(() => {
  ...
  return `此方案核保年齡 ${plan.minAge}–${plan.maxAge} 歲。依您輸入的條件，保費較基本保費${direction} ${Math.abs(pctChange)}%，預估每年保費為 ${formatCurrency(result.value.estimatedPremium)}。`
})
...
onMounted(async () => {
  try {
    ...
    if (defaultPlan) {
      ...
      age.value = Math.round((defaultPlan.minAge + defaultPlan.maxAge) / 2)
    }
  } catch {
        ...
        <div class="input-card">
          ...
          <q-form class="input-card__body" novalidate @submit.prevent="handleSubmit">
            ...
            <section class="field-group">
              <h2 class="field-group__label">
                <svg class="field-group__icon" viewBox="0 0 24 24" fill="none" aria-hidden="true">
                  ...
                  <line x1="8" y1="12" x2="16" y2="12" stroke="currentColor" stroke-width="1.6"
                    stroke-linecap="round" />
                  <line x1="8" y1="16" x2="13" y2="16" stroke="currentColor" stroke-width="1.6"
                    stroke-linecap="round" />
                </svg>
                投保方案
              </h2>
            ...
            <section class="field-group">
              <div class="slider-callout">
                <span class="field-group__label field-group__label--inline">
                  <svg class="field-group__icon" viewBox="0 0 24 24" fill="none" aria-hidden="true">
                    ...
                    <line x1="16" y1="3" x2="16" y2="6" stroke="currentColor" stroke-width="1.6"
                      stroke-linecap="round" />
                  </svg>
                  年齡
                </span>
              ...
              <q-slider v-model="age" :min="selectedPlan?.minAge ?? 0" :max="selectedPlan?.maxAge ?? 100" :step="1"
                color="primary" class="age-slider" />
            ...
            <section class="field-group">
              <h2 class="field-group__label">
                <svg class="field-group__icon" viewBox="0 0 24 24" fill="none" aria-hidden="true">
                  ...
                  <path d="M5 20c0-3.6 3-6 7-6s7 2.4 7 6" stroke="currentColor" stroke-width="1.6"
                    stroke-linecap="round" />
                </svg>
                性別
              </h2>
              <div class="btn-group gender-toggle full-width">
                <q-btn v-for="opt in [{ label: '男', value: 'M' }, { label: '女', value: 'F' }]" :key="opt.value" flat
                  no-caps :label="opt.label" :class="{ 'tgl-active': gender === opt.value }"
            ...
            <section class="field-group">
              ...
              <div class="btn-group risk-toggle full-width">
                <q-btn v-for="opt in riskLevelOptions" :key="opt.value" flat no-caps
                  :class="{ 'tgl-active': riskLevel === opt.value }" @click="riskLevel = opt.value">
            ...
            <!-- 送出按鈕 -->
            <q-btn type="submit" color="primary" :label="isLoading ? '試算中…' : '開始試算'" :loading="isLoading" no-caps
              unelevated class="full-width submit-btn" size="lg" />
          </q-form>
        </div>
